...
// ─── Knot Type → Loom Element Mapping ───────────────────────────

interface LoomMapping {
...
function getLoomMapping(knot: Knot): LoomMapping {
  switch (knot.type) {
...
function getThreadVisual(thread: Thread, sourceKnot: Knot, targetKnot: Knot): GlamourConnectionVisual {
...
function inferDataType(thread: Thread, sourceKnot: Knot): string {
...
const sceneConfig: GlamourSceneConfig = {
  background: '#0a0a0a',
  layoutMode: 'horizontal',
  spacing: { x: 300, y: 200 },
}

export const LoomTheme: GlamourTheme = {
  id: 'loom',
  name: 'The Loom',
  ...
  enchantKnot(knot: Knot, context: EnchantContext): GlamourElement {
    ...
    const visual: GlamourVisual = { type: 'svg', path: ASSETS[mapping.asset] }
